package com.abz.techlab.standalone.medium.ContainerWithMostWater;
...
class ContainerWithMostWater {
    ...
    //Time Complexity:
    //The function uses two pointers: left (starting at index 0) and right (starting at the last index).
    //In each iteration of the while(left < right), one of the pointers moves inward (either left++ or right--).
    //Since the pointers move at most n-1 times (where n is height.length), the number of iterations is O(n).

    // Space Complexity:
    //The function uses only a few integer variables (maxArea, left, right, currentArea).
    //No additional data structures (arrays, lists, hash tables, etc.) are used.
    //Since the space usage is constant and does not grow with input size, the space complexity is O(1).
    public int maxArea(int[] height) {
        ...
        //Approach with O(n square)
        // for(int left=0; left<height.length-1; left++) {
        //     for(int j=0; j<height.length; j++) {
        //         int currentArea = Math.min(height[i], height[j])*((j+1)-(i+1));
        //         maxArea = Math.max(maxArea, currentArea);
        //     }
        // }
        return maxArea;
